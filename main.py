...
def download_video():
    ...
    if video_stream:
        ...
        confirmation = tk.messagebox.askquestion("Confirmation", f"Download video?\n{size_message}")
        if confirmation == 'yes':
...
def download_playlist():
    ...
    for video in playlist.videos:
        ...
        video_stream = video.streams.filter(res=quality).first()
        if video_stream:
            total_size += get_file_size(video_stream)

    size_message = f"Total Playlist Size: {total_size} MB"
    download_complete_label.config(text=size_message)

    # Ask for confirmation
    confirmation = tk.messagebox.askquestion("Confirmation", f"Download playlist?\n{size_message}")
    if confirmation == 'yes':
        for video in playlist.videos:
            video_stream = video.streams.filter(res=quality).first()
            if video_stream:
